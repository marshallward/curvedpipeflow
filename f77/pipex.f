...
c  inf   - approximate value for upper bound y->infinity
...
c Rescale u0 for the higher orders
      do 25 i=0,m-1
         u(0,i) = u(0,i)*a
 25   continue
...
c 50   continue
      ...
      do 60 i = 0, int(inf)
         write(*,*) u(order,i*(m-1)/int(inf))*(a**(-2*order-1)),
     &              v(order,i*(m-1)/int(inf))*(a**(-2*order)),
     &              w(order,i*(m-1)/int(inf))*(a**(-2*order))
 60   continue

c      do 70 i = 0, m-1
c          write(*,*) u(order,i)*(a**(-2*order-1))
c 70   continue
...
c Local variables
     ...
     & a(3,0:9), b(3,0:9), c(3,0:9)
      ...
      parameter(wamp = 7.7971D0, vmax = 1.48323792705834D0)
...
c learn to read files someday, retard
      open(file, FILE='nodes17', STATUS='OLD')
...
c         d2u_v = ((u(0,i-1)+u(0,i-2))/2)**2
c     &          + ((v(0,i-1)+v(0,i-2))/2)*((u(0,i-1)-u(0,i-2))/h)
c     &          + (((w(0,i-1)+w(0,i-2))/2)**2 - wc(0)**2)/2
...
c Local variables
      integer i, j, r
      double precision temp, temp2, fac
...
c Setup the middle grid points of AB
      do 30 i = 2, m-1
         ...
         ab(kl+ku+1-2+2,3*i-2) = -2 - 2*(h**2)*(k+1)*u(0,i-1)/a
         ab(kl+ku+1-2+1,3*i-1) = -h/2*(u(0,i)-u(0,i-2))
         ab(kl+ku+1-2-0,3*i-0) = -(h**2)*w(0,i-1)*a
         ab(kl+ku+1-2-1,3*i+1) = 1 - h/2*v(0,i-1)
         ab(kl+ku+1-2-2,3*i+2) = 0
         ab(kl+ku+1-2-3,3*i+3) = 0

         ab(kl+ku+1-1+5,3*i-5) = (2*k+1)*h/2/a
         ab(kl+ku+1-1+4,3*i-4) = -1
         ab(kl+ku+1-1+3,3*i-3) = 0
         ab(kl+ku+1-1+2,3*i-2) = (2*k+1)*h/2/a
         ...
         ab(kl+ku+1-0+2,3*i-2) = 0
         ab(kl+ku+1-0+1,3*i-1) = -h/2*(w(0,i-0)-w(0,i-2))
         ab(kl+ku+1-0-0,3*i-0) = -2 - (h**2)*2*k*u(0,i-1)/a
...
c Setup the upper boundary of AB
      ...
      ab(kl+ku+1-1+5,3*m-5) = (2*k+1)*h/2/a
      ab(kl+ku+1-1+4,3*m-4) = -1
      ab(kl+ku+1-1+2,3*m-2) = (2*k+1)*h/2/a
...
c The 'u' part
     ...
     &                + (h**2)*(2*(k-j)+1)*u(j,i-1)*u(k-j,i-1)/a
     &                + (h**2)/2*(w(j,i-1)*w(k-j,i-1) - wc(j)*wc(k-j))*a
 50         continue
         endif
         temp = temp - (h**2)*wc(0)*wc(k)*a
...
c The 'w' part
         ...
         if(k .GT. 1) then
            do 80 j = 1, (k-1)
               temp = temp + 2*(h**2)*(k-j)*u(j,i-1)*w(k-j,i-1)/a
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine getWc(q, wc, k, order, a)
      integer k, order
      double precision q(0:order), wc(0:order), a
c Local variables
      integer i
      ...
      if(k .EQ. 1) then
         ...
         temp = ((-1)**k)/(fac(2*k-1))*(a**(2*k))
         do 10 i = 1, k-1
            temp = temp - (2*(k-i)*q(i)*wc(k-i)/a
     &       - ((-1)**k)/(fac(2*i+1)*fac(2*k-2*i-1)))*(a**(2*k))
 10      continue
         wc(k) = temp/(2*k*q(0))*a
      endif
...
c
c Hah... major problems when this factorial gets too big. Need to
c  switch to float
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
