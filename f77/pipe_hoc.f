...
c
c For Part III Maths Essay, "Fast Flow in Curved Pipes: a Conundrum?"
c set by Stephen Cowley, at the University of Cambridge, UK.
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c Variables:
c
...
c  m     - number of points on the grid (including endpoints) - *ODD*
c   (dgbsv variables)
...
c  --------------------------------
c  u     - tangential parallel component of BL velocity.
c  v     - (negative) radial component of BL velocity.
c  w     - cross-sectional component of BL velocity.
...
c 50   continue
      ...
      do 60 i = 0, int(inf)
         write(*,*) u(order,i*(m-1)/int(inf)),
     &              v(order,i*(m-1)/int(inf)),
     &              w(order,i*(m-1)/int(inf))
 60   continue
...
c
c The value of v0 was verified for inf=40, N = 20000 (up to N = 80000)
c  at O(h^4) precision
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c Local variables
     ...
     & a(3,0:9), b(3,0:9), c(3,0:9)
      ...
      parameter(wamp = 7.7971D0, vmax = 1.4832384076D0)
...
c Determine values for (u,v,w)
 ...
 60   continue
      ...
      do 80 i = 10*r, m-1
         u(0,i) = uamp*(i*h)*exp(-vmax*i*h)
         v(0,i) = -vmax + uamp/vmax*i*h*exp(-vmax*i*h)
         w(0,i) = wc0 - wamp*exp(-vmax*i*h)
 80   continue
...
c
c In fact, if my suspicion is correct and I can reduce the bandwidth
c  from 6 to 4, and then this is completely unnecessary.
      do 10 j = 1, 3*m
...
c  This compresses data by stacking diagonals on top of each other, with
c  some column offset
...
c DONE
      do 30 i = 2, m-1
         ...
         d3u = u(0,i-1)*(u(0,i)-u(0,i-2))/(2*h) + (u(0,i-1)**2)*v(0,i-1)
     &        + (v(0,i-1)**2)*(u(0,i)-u(0,i-2))/(2*h)
     &        + v(0,i-1)*(w(0,i-1)**2)/2 - (wc(0)**2)*v(0,i-1)/2
     &        + w(0,i-1)*(w(0,i)-w(0,i-2))/(2*h)

         d4u = ((u(0,i)-u(0,i-2))/(2*h))**2
     &        + (u(0,i-1)*v(0,i-1)+v(0,i-1)**3)*(u(0,i)-u(0,i-2))/(2*h)
     &        + (u(0,i-1)**2+(w(0,i-1)**2-wc(0)**2)/2)*v(0,i-1)**2
     &        + 2*v(0,i-1)*w(0,i-1)*(w(0,i)-w(0,i-2))/(2*h)
     &        + ((w(0,i)-w(0,i-2))/(2*h))**2

         ab(kl+ku+1-2+5,3*i-5) = 1 + h/2*v(0,i-1)
     &                       + (h**2)/24*(u(0,i)-u(0,i-2))
     &                       + (h**3)/24*(u(0,i-1)*v(0,i-1)-v(0,i-1)**3)
     &                       - (h**3)/12*(u(0,i-1)*v(0,i-1)-v(0,i-1)**3)
         ab(kl+ku+1-2+4,3*i-4) = 0
         ab(kl+ku+1-2+3,3*i-3) = (h**2)/24*(w(0,i)-w(0,i-2))
     &                          + (h**3)/12*(v(0,i-1)*w(0,i-1))
     &                          - (h**2)/12*(v(0,i-1)*w(0,i-1))
         ab(kl+ku+1-2+2,3*i-2) = -2 - 2*(h**2)*u(0,i-1)
     &                          - (h**3)/24*v(0,i-1)*(u(0,i)-u(0,i-2))
     &                          - (h**4)/6*u(0,i-1)*(v(0,i-1)**2)
     &                          + (h**3)/12*v(0,i-1)*(u(0,i)-u(0,i-2))
     &                          + (h**4)/3*u(0,i-1)*(v(0,i-1)**2)
         ab(kl+ku+1-2+1,3*i-1) = -h/2*(u(0,i-0)-u(0,i-2))
     &                          - (h**3)/24*u(0,i-1)*(u(0,i)-u(0,i-2))
     &                          - (h**4)/6*v(0,i-1)*(u(0,i-1)**2
     &                           + (w(0,i-1)**2)/2 - (wc(0)**2)/2)
     &                          - (h**3)/12*w(0,i-1)*(w(0,i)-w(0,i-2))
     &                          + (h**4)/6*d3u
     &                          + (h**4)/6*v(0,i-1)*(u(0,i-1)**2)
     &                          + (h**3)/6*v(0,i-1)*(u(0,i)-u(0,i-2))
     &                          + (h**4)/12*(w(0,i-1)**2 - wc(0)**2)
         ab(kl+ku+1-2-0,3*i-0) = -(h**2)*w(0,i-1)
     &                          - (h**4)/12*w(0,i-1)*(v(0,i-1)**2)
     &                          - (h**3)/12*v(0,i-1)*(w(0,i)-w(0,i-2))
     &                          + (h**4)/6*w(0,i-1)*(v(0,i-1)**2)
     &                          + (h**3)/12*v(0,i-1)*(w(0,i)-w(0,i-2))
     ...
     &                          + (h**3)/12*(v(0,i-1)*w(0,i-1))
         ...
         ab(kl+ku+1-1+5,3*i-5) = h/2 + (h**2)/24*(v(0,i-1)+v(0,i-2))
     &                             - (h**3)/24*(u(0,i-1)+u(0,i-2))
         ab(kl+ku+1-1+4,3*i-4) = -1 - (h**2)/24*(u(0,i-1)-u(0,i-2))
         ab(kl+ku+1-1+3,3*i-3) = -(h**3)/48*(w(0,i-1)+w(0,i-2))
         ab(kl+ku+1-1+2,3*i-2) = h/2 - (h**2)/24*(v(0,i-1)+v(0,i-2))
     &                             - (h**3)/24*(u(0,i-1)+u(0,i-2))
         ab(kl+ku+1-1+1,3*i-1) = 1 - (h**2)/24*(u(0,i-1)-u(0,i-2))
         ab(kl+ku+1-1-0,3*i-0) = -(h**3)/48*(w(0,i-1)+w(0,i-2))
         ...
         ab(kl+ku+1-0+5,3*i-5) = -(h**2)/48*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0+4,3*i-4) = 0
         ab(kl+ku+1-0+3,3*i-3) = 1 + h/2*v(0,i-1)
     &                             - (h**2)/48*(u(0,i)-u(0,i-2))
     &                             - (h**3)/24*u(0,i-1)*v(0,i-1)
     &                             - (h**3)/24*(v(0,i-1)**3)
         ab(kl+ku+1-0+2,3*i-2) = (h**3)/24*v(0,i-1)*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0+1,3*i-1) = -h/2*(w(0,i-0)-w(0,i-2))
     &                     - (h**3)/24*u(0,i-1)*(w(0,i)-w(0,i-2))
     &                     + 3*(h**3)/8*(v(0,i-1)**2)*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0-0,3*i-0) = -2
         ab(kl+ku+1-0-1,3*i+1) = (h**2)/48*(w(0,i)-w(0,i-2))
...
c Setup the upper boundary of AB
      ...
      ab(kl+ku+1-1+5,3*m-5) = h/2 + (h**2)/24*(v(0,m-1)+v(0,m-2))
     &                             - (h**3)/24*(u(0,m-1)+u(0,m-2))
      ab(kl+ku+1-1+4,3*m-4) = -1 - (h**2)/24*(u(0,m-1)-u(0,m-2))
      ab(kl+ku+1-1+3,3*m-3) = -(h**3)/48*(w(0,m-1)+w(0,m-2))
      ab(kl+ku+1-1+2,3*m-2) = h/2 - (h**2)/24*(v(0,m-1)+v(0,m-2))
     &                             - (h**3)/24*(u(0,m-1)+u(0,m-2))
      ab(kl+ku+1-1+1,3*m-1) = 1 - (h**2)/24*(u(0,m-1)-u(0,m-2))
      ab(kl+ku+1-1-0,3*m-0) = -(h**3)/48*(w(0,m-1)+w(0,m-2))
...
c Fix this
     ...
     &                       + (h**4)/12*d4u - (h**4)/6*v(0,i-1)*d3u
         b(3*i-1,1) = -(v(0,i-1) - v(0,i-2))
     &               - h/2*(u(0,i-1)+u(0,i-2))
     &               + (h**2)/12*(h/4*((u(0,i-1)+u(0,i-2))**2)
     &                 + (v(0,i-1)+v(0,i-2))*(u(0,i-1)-u(0,i-2))/2
     &                 + h/2*((((w(0,i-1)+w(0,i-2))/2)**2)-wc(0)**2))
         b(3*i-0,1) = -w(0,i-2) + 2*w(0,i-1) - w(0,i)
     &               + h/2*v(0,i-1)*(w(0,i) - w(0,i-2))
     &               - (h**2)/48*(u(0,i)-u(0,i-2))*(w(0,i)-w(0,i-2))
     &               - (h**3)/24*(3*u(0,i-1)*v(0,i-1) - v(0,i-1)**3)
     &                    * (w(0,i)-w(0,i-2))
     &               - (h**3)/12*(v(0,i-1)**3 - u(0,i-1)*v(0,i-1))
     &                    * (w(0,i)-w(0,i-2))
 40   continue

      b(3*m-2,1) = 0
      b(3*m-1,1) = -(v(0,m-1) - v(0,m-2)) - h/2*(u(0,m-1)+u(0,m-2))
     &             + (h**2)/12*(h/4*((u(0,m-1)+u(0,m-2))**2)
     &                 + (v(0,m-1)+v(0,m-2))*(u(0,m-1)-u(0,m-2))/2
     &                 + h/2*((((w(0,m-1)+w(0,m-2))/2)**2)-wc(0)**2))
      b(3*m-0,1) = 0
...
c Local variables
      integer i, j, r
      double precision temp, temp2, fac, usum1, dusum1, d2usum1, usum2,
     &     vsum1
...
ccccccccc
         ab(kl+ku+1-2+5,3*i-5) = 1 + h/2*v(0,i-1)
     &                   + (4*k+1)*(h**3)/24*u(0,i-1)*v(0,i-1)
     &                   + (k+1)*(h**2)/24*(u(0,i)-u(0,i-2))
     &                   + (h**3)/24*(v(0,i-1)**3)
     &               - (2*k+1)*(h**3)/12*u(0,i-1)*v(0,i-1)
     &               - (h**3)/12*(v(0,i-1)**3)
         ab(kl+ku+1-2+4,3*i-4) = 0
         ab(kl+ku+1-2+3,3*i-3) = 0
     &                   + (h**3)/12*v(0,i-1)*w(0,i-1)
     &                   + (h**2)/24*(w(0,i)-w(0,i-2))
     &               - (h**3)/12*v(0,i-1)*w(0,i-1)
        ab(kl+ku+1-2+2,3*i-2) = -2 - 2*(h**2)*(k+1)*u(0,i-1)
     &                   + (2*k-1)*(h**3)/24*v(0,i-1)*(u(0,i)-u(0,i-2))
     &                   - k*(2*k+1)*(h**4)/6*(u(0,i-1)**2)
     &                   - (k+1)*(h**4)/6*u(0,i-1)*(v(0,i-1)**2)
     &                   + k*(h**4)/12*((w(0,i-1)**2)-(wc(0)**2))
     &               + (k+1)*(h**4)/3*u(0,i-1)*(v(0,i-1)**2)
     &               + (h**3)/12*v(0,i-1)*(u(0,i)-u(0,i-2))
         ab(kl+ku+1-2+1,3*i-1) = -h/2*(u(0,i-0)-u(0,i-2))
     &                   - (h**4)/6*(u(0,i-1)**2)*v(0,i-1)
     &                   - (2*k+1)*(h**3)/24*u(0,i-1)*(u(0,i)-u(0,i-2))
     &                   - (h**3)/8*(v(0,i-1)**2)*(u(0,i)-u(0,i-2))
     &                   - (h**4)/12*v(0,i-1)*((w(0,i-1)**2)-(wc(0)**2))
     &                   - (h**3)/12*w(0,i-1)*(w(0,i)-w(0,i-2))
     &               + (h**3)/6*(v(0,i-1)**2)*(u(0,i)-u(0,i-2))
     &               + (h**4)/6*v(0,i-1)*(u(0,i-1)**2)
     &               + (h**4)/12*v(0,i-1)*((w(0,i-1)**2)-(wc(0)**2))
     &           + (h**4)/6*(u(0,i-1)*(u(0,i)-u(0,i-2))/(2*h)
     &            + (u(0,i-1)**2)*v(0,i-1)
     &            + (v(0,i-1)**2)*(u(0,i)-u(0,i-2))/(2*h)
     &            + v(0,i-1)*((w(0,i-1)**2)-(wc(0)**2))/2
     &            + w(0,i-1)*(w(0,i)-w(0,i-2))/(2*h))
         ab(kl+ku+1-2-0,3*i-0) = -(h**2)*w(0,i-1)
     &                   - k*(h**4)/3*u(0,i-1)*w(0,i-1)
     &                   - (h**3)/12*v(0,i-1)*(w(0,i)-w(0,i-2))
     &                   - (h**4)/12*(v(0,i-1)**2)*w(0,i-1)
     &               + (h**4)/6*(v(0,i-1)**2)*w(0,i-1)
     &               + (h**3)/12*v(0,i-1)*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-2-1,3*i+1) = 1 - h/2*v(0,i-1)
     &                   - (4*k+1)*(h**3)/24*u(0,i-1)*v(0,i-1)
     &                   - (k+1)*(h**2)/24*(u(0,i)-u(0,i-2))
...
ccccccccc
     ...
     &                    + (3*k+1)*(h**2)/24*(v(0,i-1)+v(0,i-2))
     &                    - (3*k+1)*(k+1)*(h**3)/24*(u(0,i-1)+u(0,i-2))
         ab(kl+ku+1-1+4,3*i-4) = -1
     &                    - (3*k+1)*(h**2)/24*(u(0,i-1)-u(0,i-2))
         ab(kl+ku+1-1+3,3*i-3) = 0
     &                    - (3*k+1)*(h**3)/48*(w(0,i-1)+w(0,i-2))
...
c **w**
ccccccccc
         ab(kl+ku+1-0+5,3*i-5) = -(2*k+1)*(h**2)/48*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0+4,3*i-4) = 0
         ab(kl+ku+1-0+3,3*i-3) = 1 + h/2*v(0,i-1)
     &                          + (h**2)/48*(4*k-1)*(u(0,i)-u(0,i-2))
     &                          + (h**3)/24*(4*k-3)*u(0,i-1)*v(0,i-1)
     &                          + (h**3)/24*(v(0,i-1)**3)
     &     - (h**3)/12*v(0,i-1)*((2*k-1)*u(0,i-1) + v(0,i-1)**2)

         ab(kl+ku+1-0+2,3*i-2) = 0
     &                   + (2*k+1)*(h**3)/8*v(0,i-1)*(w(0,i)-w(0,i-2))
     &     - (2*k+1)*(h**3)/12*v(0,i-1)*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0+1,3*i-1) = -h/2*(w(0,i-0)-w(0,i-2))
     &                   - (2*k-3)*(h**3)/24*u(0,i-1)*(w(0,i)-w(0,i-2))
     &                   - (h**3)/8*(v(0,i-1)**2)*(w(0,i)-w(0,i-2))
     &     + (h**3)/6*(v(0,i-1)**2)*(w(0,i)-w(0,i-2))
     &     + (h**3)/12*((v(0,i-1)**2)-u(0,i-1))*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0-0,3*i-0) = -2 - (h**2)*2*k*u(0,i-1)
     &                   - k*(h**3)/6*v(0,i-1)*(u(0,i)-u(0,i-2))
     &                   - k*(h**4)/12*((w(0,i-1)**2)-(wc(0)**2))
     &                   - k*(h**4)/6*((2*k-1)*(u(0,i-1)**2) +
     &                                 + u(0,i-1)*(v(0,i-1)**2))
     &     + k*(h**3)/6*v(0,i-1)*(u(0,i)-u(0,i-2))
     &     + k*(h**4)/3*u(0,i-1)*(v(0,i-1)**2)
         ab(kl+ku+1-0-1,3*i+1) = (2*k+1)*(h**2)/48*(w(0,i)-w(0,i-2))
         ab(kl+ku+1-0-2,3*i+2) = 0
         ab(kl+ku+1-0-3,3*i+3) = 1 - h/2*v(0,i-1)
     &                          - (h**2)/48*(4*k-1)*(u(0,i)-u(0,i-2))
     &                          - (h**3)/24*(4*k-3)*u(0,i-1)*v(0,i-1)
...
c Setup the upper boundary of AB
     ...
     &                    + (3*k+1)*(h**2)/24*(v(0,m-1)+v(0,m-2))
     &                    - (3*k+1)*(k+1)*(h**3)/24*(u(0,m-1)+u(0,m-2))
         ab(kl+ku+1-1+4,3*m-4) = -1
     &                    - (3*k+1)*(h**2)/24*(u(0,m-1)-u(0,m-2))
         ab(kl+ku+1-1+3,3*m-3) = -(3*k+1)*(h**3)/48*(w(0,m-1)+w(0,m-2))
         ab(kl+ku+1-1+2,3*m-2) = (2*k+1)*h/2
     &                    - (3*k+1)*(h**2)/24*(v(0,m-1)+v(0,m-2))
     &                    - (3*k+1)*(k+1)*(h**3)/24*(u(0,m-1)+u(0,m-2))
         ab(kl+ku+1-1+1,3*m-1) = 1
     &                    - (3*k+1)*(h**2)/24*(u(0,m-1)-u(0,m-2))
         ab(kl+ku+1-1-0,3*m-0) = -(3*k+1)*(h**3)/48*(w(0,m-1)+w(0,m-2))
...
c The 'u' part
ccccccccccccccc
cc DONT FORGET TO ADD S2's and S1w hehe
         usum1 = 0
         do 42 j = 1, k
            temp = 0
            do 52 r = 0, (k-j)
               temp = temp + w(r,i-1)*w(k-j-r,i-1) - wc(r)*wc(k-j-r)
 52         continue
            usum1 = usum1 + ((-1)**j)/(2*fac(2*j+1))*temp
 42      continue
         ...
         do 44 j = 1, k
            temp = 0
            do 54 r = 0, (k-j)
               temp = temp + (w(r,i)-w(r,i-2))*w(k-j-r,i-1)/(2*h)
     &                     + w(r,i-1)*(w(k-j-r,i)-w(k-j-r,i-2))/(2*h)
 54         continue
            dusum1 = dusum1 + ((-1)**j)/(2*fac(2*j+1))*temp
 44      continue
...
c missing S1w's
         do 46 j = 1, k
            ...
            do 56 r = 0, (k-j)
               temp = temp
     &        + (2*r*u(0,i-1)*w(r,i-1)+v(0,i-1)*(w(r,i)-w(r,i-2))/(2*h)
     &                +(w(r,i)-w(r,i-2))/(2*h)*v(r,i-1))*w(k-j-r,i-1)
     &        + (w(r,i)-w(r,i-2))*(w(k-j-r,i)-w(k-j-r,i-2))/(2*(h**2))
     &        + w(r,i-1)*(2*(k-j-r)*u(0,i-1)*w(k-j-r,i-1)
     &               +v(0,i-1)*(w(k-j-r,i)-w(k-j-r,i-2))/(2*h)
     &               +(w(0,i)-w(0,i-2))/(2*h)*v(k-j-r,i-1))
 56         continue
            d2usum1 = d2usum1 + ((-1)**j)/(2*fac(2*j+1))*temp
 46      continue

         usum2 = 0
         if(k .GT. 1) then
            do 70 j = 1, (k-1)
               usum2 = usum2 + h/2*v(j,i-1)*(u(k-j,i-0)-u(k-j,i-2))
     &                + (h**2)*(2*(k-j)+1)*u(j,i-1)*u(k-j,i-1)
     &                + (h**2)/2*(w(j,i-1)*w(k-j,i-1) - wc(j)*wc(k-j))
 70         continue
         endif

         b(3*i-2,1) = -(h**2)*wc(0)*wc(k) + (h**2)*usum1
     &       - (h**4)/6*v(0,i-1)*(v(0,i-1)*(usum1-wc(0)*wc(k))+dusum1)
     &       + (h**4)/12*((2*k*u(0,i-1)+(v(0,i-1)**2))
     &                                      *(usum1-wc(0)*wc(k))
     &                  + v(0,i-1)*dusum1 + d2usum1)
...
c            do 80 j = 1, (k-1)
c               temp = temp
c     &      + (v(j,i-1)+v(j,i-2))*(u(k-j,i-i)-u(k-j,i-2))/(2*h)
c     &      + (2*(k-j)+1)*(u(j,i-1)+u(j,i-2))*(u(k-j,i-1)+u(k-j,i-2))/4
c     &      + ((w(j,i-1)+w(j,i-2))*(w(k-j,i-1)+w(k-j,i-2))/4
c     &                                              - wc(j)*wc(k-j))/2
c 80         continue
c         endif
c         temp = temp - wc(0)*wc(k)
         ...
         do 90 j = 1, k
            ...
            do 100 r = 0, (k-j)
               temp = temp
     &               + (w(r,i-1)+w(r,i-2))*(w(k-j-r,i-1)+w(k-j-r,i-2))/4
     &                       - wc(r)*wc(k-j-r)
 100        continue
            vsum1 = vsum1 + ((-1)**j)/(2*fac(2*j+1))*temp
 90      continue

         b(3*i-1,1) = (3*k+1)*(h**3)/12*(vsum1 - wc(0)*wc(k))
...
c REMEMBER TO ADD THE S1's from both w''' and w''''
c   (NB: i think there are 5!!!)
         b(3*i-0,1) = temp

 40   continue

c Set B at the outer end
      b(3*m-2,1) = 0
      ...
      do 120 j = 1, k
            ...
            do 130 r = 0, (k-j)
               temp = temp
     &            + (w(r,m-1)+w(r,m-2))*(w(k-j-r,m-1)+w(k-j-r,m-2))/4
     &                    - wc(r)*wc(k-j-r)
 130     continue
         vsum1 = vsum1 + ((-1)**j)/(2*fac(2*j+1))*temp
 120  continue
      b(3*m-1,1) = (3*k+1)*(h**3)/12*(vsum1 - wc(0)*wc(k))
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c getWc - calculates the kth order wc term based on k-1 terms
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c
c Hah... major problems then this factorial gets too big. Need to switch
c  to float
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
